...
- 승인 후 provider 실행이 실패하면 승인 요청은 `pending`으로 돌아갑니다. 실패 증적에는 sanitize된 오류 유형,
  ...
  관리자가 provider 설정을 고친 뒤 `POST /v1/approvals/{approval_id}/reset-execution-error`에 사유를 남겨야
  `can_resolve`가 다시 열립니다. reset 내역은 `approval_execution_error_reset` 감사 이벤트로 기록됩니다.
