...
- OpenAI 호환 `/v1/chat/completions` 게이트웨이
...
```mermaid
flowchart LR
    User["사용자 또는 AI 앱"] --> Gateway["OpenAI 호환 게이트웨이"]
...
관리자와 승인자 토큰을 설정합니다. 실제 운영 토큰은 절대 Git에 커밋하지 마세요.
...
엔드포인트를 설정하지 않으면 로컬 echo 제공자를 사용합니다.
...
- 스트리밍 채팅 응답 pass-through는 아직 없습니다.
...
1. Gateway 하드닝: 스트리밍, provider retry budget, 운영 인증, 보존 정책, 암호화 증적 분리
