...
sequenceDiagram
    ...
    alt 승인 필요
        ...
        else 재시도 가능한 provider 실패
            M-->>GW: 일시적 provider 실패
            GW->>Q: 승인 요청을 pending으로 복귀
            GW->>A: approval_execution_failed 기록
            GW-->>Admin: 502, 승인 요청은 pending으로 유지
        else 재시도 불가 provider 실패
            M-->>GW: 401, 403, 422 등 provider 오류
            GW->>Q: pending 유지, 재실행 잠금
            GW->>A: retryable=false 실패 증적 기록
            Admin->>GW: reason_code와 함께 실행 오류 reset
            GW->>A: approval_execution_error_reset 기록
        end
    else 허용 경로
...
- 승인 후 provider 실행은 임시 `executing` 상태와 idempotency key를 사용하므로 중복 승인 요청이
  중복 모델 응답을 만들지 않습니다. 성공/실패 상태 전이는 attempt-aware 방식이므로 오래된 provider callback이
  새 실행 attempt를 승인하거나 실패 처리할 수 없습니다.
- provider 호출 이후 오래된 attempt가 현재 승인 상태와 충돌하면 구조화된 `409` 응답을 반환하고,
  새 승인 상태를 바꾸지 않은 채 `approval_execution_attempt_conflict` 감사 이벤트를 남깁니다.
- 승인 후 provider 실행이 실패하면 승인 요청은 `pending`으로 돌아갑니다. 실패 증적에는 sanitize된 오류 유형,
  provider 상태 코드, 시도 횟수, cap된 provider 오류 본문 hash, 본문 truncation 여부, 상태 코드 기반 재시도
  가능 여부가 남습니다. `retryable=false`인 provider 실패는 바로 재승인할 수 없고,
  관리자가 provider 설정을 고친 뒤 `POST /v1/approvals/{approval_id}/reset-execution-error`에
  `reason_code`를 남겨야 `can_resolve`가 다시 열립니다. reset 내역은
  `approval_execution_error_reset` 감사 이벤트로 기록됩니다.
- reset은 `provider_http_error`, `provider_timeout`, `provider_invalid_response`,
  `provider_runtime_error`에만 기본 허용됩니다. gateway state/runtime, approval backend, unknown 오류는
  단순 reset으로 다시 열지 않습니다.
- reset 자유 코멘트는 선택 입력입니다. 입력되더라도 200자로 제한되고, 명백한 secret/URL은 redaction된 뒤
  SHA-256 hash만 감사 이벤트에 남습니다. 증적 패키지 timeline에는 기본적으로 `reason_code`만 노출합니다.
- 저장된 승인 context 검증 실패는 provider 재시도 문제가 아니라 gateway 상태 오류이므로,
  non-retryable `stored_approval_context_error`로 기록하고 provider를 호출하지 않습니다.
  이 경우 구조화된 `409` 응답을 반환하고 승인 상태를 `invalid_context`로 바꾸며,
  관리자 승인 payload에는 `retryable=false`와 `recommended_action=operator_review`가 표시됩니다.
  항목은 관리자 화면에 계속 보이지만 재승인 버튼은 비활성화되고, 반려 또는 운영자 검토 대상으로 남습니다.
- 승인 payload는 운영자 동작 가능 여부를 `can_resolve`, `can_execute_provider`,
  `can_reject`, `resolution_mode`으로 나눠 표시합니다. 기존 `can_execute`는
  호환을 위해 `can_resolve` 별칭으로 유지합니다.
- 승인 실패 증적에는 `gateway_state`, `gateway_runtime`, `approval_backend`,
  `provider_transport`, `provider_response`, `approval_state_conflict`, `unknown` 같은
  `failure_domain`을 남겨 SIEM/리포트 필터링을 쉽게 합니다.
- `/v1/security/evaluate`가 만든 정책 평가 전용 승인은 명시적인 `policy_evaluation` context를 사용합니다.
  따라서 context 누락이나 알 수 없는 context type이 더 이상 조용히 성공 처리되지 않습니다.
...
- 관리자는 `POST /v1/approvals/recover-stale`로 오래된 in-memory `executing` 승인 요청을 `pending`으로
  되돌릴 수 있습니다. 복구 내역은 복구한 관리자 신원과 함께 `approval_execution_stale_recovered` 이벤트로
  기록됩니다. `dry_run=true`로 대상만 미리 확인할 수 있고, 기본값보다 짧은 timeout에는 `force=true`가 필요합니다.
...
- reset 상태 변경과 reset 감사 이벤트 append는 아직 단일 트랜잭션이 아닙니다. 운영형 v0.2에서는
  approval 상태 전이와 audit/outbox 기록을 같은 트랜잭션으로 묶어야 합니다.
- provider idempotency key는 아직 attempt 단위입니다. 후속 영속 실행 ledger에서는 논리적 approval idempotency,
  attempt 단위 감사 증적, unknown-outcome 처리를 분리해야 합니다.
