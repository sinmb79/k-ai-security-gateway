...
- 승인 후 provider 실행이 실패하면 승인 요청은 `pending`으로 돌아가 재시도할 수 있습니다. 실패 증적에는
  sanitize된 오류 유형, provider 상태 코드, 시도 횟수, cap된 provider 오류 본문 hash, 본문 truncation 여부,
  상태 코드 기반 재시도 가능 여부가 남습니다.
...
- 승인 실패 증적에는 `gateway_state`, `provider_transport`, `provider_response`,
  `approval_state_conflict` 같은 `failure_domain`을 남겨 SIEM/리포트 필터링을 쉽게 합니다.
